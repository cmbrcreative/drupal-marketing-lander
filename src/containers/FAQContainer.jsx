...
const FAQContainer = () => {
  ...
  return (
    <div className="relative mx-auto max-w-[74.4rem]">
      ...
      <div className="absolute -bottom-[14rem] sm:-bottom-[8.4rem] w-[102%] h-[12rem] sm:h-[6.4rem] -right-3 t-fade-bottom z-10"></div>
      <dl className="border-1 after:border-1 relative mt-10 space-y-6 border-gray-200 px-[4rem] before:absolute before:right-0 before:top-0 before:h-[125%] before:w-0 before:-translate-y-[10%] before:border-r before:border-dashed after:absolute after:left-0 after:top-0 after:h-[125%] after:w-0 after:-translate-y-[10%] after:border-l after:border-dashed after:outline-gray-200">
        {constants.faqs.map((faq, index) => {
          const isLast = index === constants.faqs.length - 1;
          return (
            <Disclosure
              key={faq.question}
              as="div"
              ...
              defaultOpen={index === 0}
              onChange={() => togglePanels({ key: index, open: true, close: () => {} }, index)}
            >
              {(panel) => {
                      ...
                      <AnimatePresence initial={false}>
                        {open && (
                          <motion.div
                            ...
                            transition={{ height: { duration: prefersReducedMotion ? 0 : 0.3, ease: easeOut }, opacity: { duration: prefersReducedMotion ? 0 : 0.2, ease: easeOut } }}
