...
import { principles } from "./principlesConstants";
      ...
      >
        {principles.map((principle, index) => (
          <SwiperSlide key={index}>
            <div className="flex max-w-[60rem] flex-col gap-[2.4rem] text-left lg:max-w-[80rem]">
              <h3 className="font-rebond text-[4rem] font-medium leading-[100%] text-white lg:text-[9.6rem]">
                {principle.title}
              </h3>
              <p className="p-sm-reg lg:p-xl-reg text-white">
                {principle.description}
              </p>
              {principle.hasCta && principle.ctaComponent}
            </div>
          </SwiperSlide>
        ))}
